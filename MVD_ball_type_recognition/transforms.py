...
class GroupMultiScaleTwoResizedCrop(object):

    def __init__(self, input_size, input_size_1, scales=None, max_distort=1, fix_crop=True, more_fix_crop=True):
        self.scales = scales if scales is not None else [1, .875, .75, .66]
        self.max_distort = max_distort
        self.fix_crop = fix_crop
        self.more_fix_crop = more_fix_crop
        self.input_size = input_size if not isinstance(input_size, int) else [input_size, input_size]
        self.input_size_1 = input_size_1 if not isinstance(input_size_1, int) else [input_size_1, input_size_1]
        self.interpolation = Image.BILINEAR
    ...
    def __call__(self, img_tuple):
        ...
        crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(im_size)
        # crop_img_group = [img.crop((offset_w, offset_h, offset_w + crop_w, offset_h + crop_h)) for img in img_group]
        crop_img_group = [img for img in img_group]
        ret_img_group = [img.resize((self.input_size[0], self.input_size[1]), self.interpolation) for img in
                         crop_img_group]
        ret_img_group_1 = [img.resize((self.input_size_1[0], self.input_size_1[1]), self.interpolation) for img in
                         crop_img_group]
        return (ret_img_group, ret_img_group_1, label)
