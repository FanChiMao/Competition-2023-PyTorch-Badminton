...
class SSVideoClsDataset(Dataset):
    ...
    def loadvideo_decord(self, sample, sample_rate_scale=1):
        ...
        if self.mode == 'test':
            all_index = []
            tick = float(len(vr) - 1) / float(self.num_segment)
            interval_cross_view = tick / self.test_num_segment
            for i in range(self.num_segment):
                for ci in range(self.test_num_segment):
                    start = int(np.round(tick * i))
                    end = int(np.round(tick * (i + 1)))
                    if self.test_num_segment > 1:
                        start = int(np.round(tick * i + interval_cross_view * ci))
                        end = int(np.round(tick * i + interval_cross_view * (ci + 1)))
                        all_index.append((start + end) // 2)
                    else:
                        all_index.append((start + end) // 2)
                        all_index.append((start + end) // 2)
        ...
        average_duration = float(len(vr) - 1) / self.num_segment
        all_index = []
        for i in range(self.num_segment):
