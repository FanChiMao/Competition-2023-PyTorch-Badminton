...
class CustomDataset(object):
    def __init__(self, keys: List[str]):
        ...
        # print([key for key in self.datasets.keys()])
    ...
    def __getitem__(self, index):
        ...
        seq = video_file['features'][...].astype(np.float32)
        hitframe_gt = video_file['hitframe_gt'][...].astype(np.float32)
        direction_gt = video_file['direction_gt'][...].astype(np.float32)
        cps = video_file['change_points'][...].astype(np.int32)
        n_frames = video_file['n_frames'][...].astype(np.int32)
        nfps = video_file['n_frame_per_seg'][...].astype(np.int32)
        picks = video_file['picks'][...].astype(np.int32)
        ...
        hitframe_gt -= hitframe_gt.min()
        max = hitframe_gt.max()
        if max == 0:
            print(key)
        hitframe_gt /= hitframe_gt.max()
        # direction_gt -= direction_gt.min()
        # direction_gt /= direction_gt.max()

        return key, seq, hitframe_gt, direction_gt, cps, n_frames, nfps, picks, user_summary, video_name
